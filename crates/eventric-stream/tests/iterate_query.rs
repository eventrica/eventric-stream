...
fn iterate_query_by_multiple_identifiers() -> Result<(), Error> {
    ...
    assert_eq!(
        events.len(),
        3,
        "Should match course_created and course_updated events"
    );
...
fn iterate_query_by_identifier_and_tags() -> Result<(), Error> {
    ...
    assert_eq!(
        events.len(),
        2,
        "Should match student_enrolled events with course:200"
    );
    assert_eq!(events[0].position(), &Position::new(0));
    assert_eq!(events[1].position(), &Position::new(2));
...
fn iterate_query_from_position() -> Result<(), Error> {
    ...
    assert_eq!(
        events.len(),
        1,
        "Should match student_enrolled events from position 3"
    );
...
fn iterate_query_with_multiple_selectors() -> Result<(), Error> {
    ...
    assert_eq!(
        events.len(),
        3,
        "Should match course_created OR (student_enrolled with student:100)"
    );
    assert_eq!(events[0].position(), &Position::new(0));

...
/// pre- and post- stream append, as well as types related to specifying events
/// within queries.
pub mod event {
...
pub mod stream {
    /// The [`append`] module contains types related to append operations on the
    /// core [`Stream`]. Note that append operations (when conditional) rely on
    /// queries, and thus will also likely require types from the [`query`]
    /// module.
    pub mod append {
        pub use eventric_core_stream::append::condition::Condition;
    }

    /// The [`query`] module contains types related to query operations on the
    /// core [`Stream`].
    pub mod query {

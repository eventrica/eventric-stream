...
pub fn main() -> Result<(), Box<dyn Error>> {
    ...
    let student_stream =
        index::operation::descriptor::forward::iterate(&read, None, &student_specifier);

    let course_specifier = Specifier::new("CourseCapacityChanged", None).into();
    let course_stream =
        index::operation::descriptor::forward::iterate(&read, None, &course_specifier);

    let student_or_course_stream =
        eventric_core_util::iter::or::sequential_or([student_stream, course_stream]);
    ...
    let course_tag_stream = index::operation::tags::forward::iterate(&read, None, &course_tag);

    let student_tag = Tag::new("student:3242").into();
    let student_tag_stream = index::operation::tags::forward::iterate(&read, None, &student_tag);

    let course_tag_and_student_tag_stream =
        eventric_core_util::iter::or::sequential_or([course_tag_stream, student_tag_stream]);
